...
var ENTER = 13, SPACE = 32, ESC = 27, BACKSPACE = 8, SHIFT = 16, SWITCH = 96, FINISH = 9999; // `
var $target, $cursor, $viMode=true, $initialized=false, $byPassKeypress=false, $logging = false, $isVietnamese, $showSingle = true, $preViMode = true;
...
vinova.process = function ( event ) {
  ...
  if ( this.raw.length > 10 ) { $viMode = false; return; }
  
  this.update(); 
  $isVietnamese = this.isViSyllable( this.mix );

  if (charCode === SWITCH || charCode === BACKSPACE || this.raw.length > 1) {
    if (charCode !== SWITCH) {
      $viMode = $isVietnamese || ( $preViMode && !enSpellCheck( this.raw ) );
    }
