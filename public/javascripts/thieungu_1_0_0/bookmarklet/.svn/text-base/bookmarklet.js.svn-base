var TN_SERVER_BASE_URL = "http://localhost:3000/";
//var TN_SERVER_BASE_URL = "http://thieungu.com/dict/";
 ...
 */
	...
	function createBuble ( text ) {
		...
		if (text == "")
			// jQuery("#bubble-text").html("Empty word is detected!");
			return;
		else { 
				jQuery("body").append("<div style = 'position: absolute; left: " + xBubblePos + "px; top: " + yBubblePos + "px;"
				+ "width: 288px; height: 144px; opacity: 1; background:url(" + TN_SERVER_BASE_URL + "images/logos/bubble_bookmarklet.png) no-repeat; cursor: pointer !important;' id = 'bubble'>" +
				"<div style = 'position: relative;top: 10px; font-size: 1.3em !important;width: 260px;font-family: Verdana,Arial,Helvetica,sans-serif !important;text-align:center !important;' ext:qtip='Close me'><b>" + text + "</b></div><div style = 'position: relative; left: 190px; top: -1px; cursor: pointer;'><div style = 'display:inline;' onmousedown = 'javascript:jQuery(\"div\").remove(\"#bubble\");'><img src = '" + TN_SERVER_BASE_URL + "images/icons/cross.png' border = '0' width = '12'></img></div></div><div id = 'bubble-text' style = 'font-family: Verdana,Arial,Helvetica,sans-serif !important; cursor:pointer !important;font-size: 0.7em !important; font-size: 11px !important; text-align:left !important; position: relative;top: 2px; left: 20px; font-size: 1em !important;width: 236px;'><div style = 'height:8px;'></div><img src = '" + TN_SERVER_BASE_URL + "javascripts/ext_2.0a/resources/images/default/grid/grid-loading.gif'> Searching ...</div></div></div>");
				jQuery("#bubble").ifixpng();
				
				thieungudata = null;
				var script = document.createElement('script');
				script.src = TN_SERVER_BASE_URL + "dict/find_bookmarklet?query=" + text;
				document.getElementsByTagName('head')[0].appendChild(script);
					
				// data is already enabled
	...
	function waitToLoadData() {
		...
		if (thieungudata == null || typeof thieungudata == "undefined")
			setTimeout(function() {
				waitToLoadData();
			}, 1000);
		else {
					
					var summary = "<br/>This word cannot be found!";
					if (thieungudata.summary != "")
						summary = thieungudata.summary.substring(0, 150) + "...<br/>";
